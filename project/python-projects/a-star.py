...
def a_star(screen):
    ...
    drawastart(screen, cpath, points)

    time.sleep(200)


def drawastart(screen, path, points):
    first = path[0]
    for p in path:
        pygame.draw.line(screen, (0, 255, 0), points[first], points[p], 8)
        first = p
...
def drawOnce(screen, points, edges):
    ...
    pygame.draw.circle(screen, (255, 0, 0), points[9], 10)
