...
class ball {
  ...
  void draw() {
    PVector back = location.copy().add(direction.copy().normalize().mult(-size));
    PVector left = back.copy().add(direction.copy().normalize().rotate(HALF_PI).mult(size/3));
    PVector right = back.copy().add(direction.copy().normalize().rotate(HALF_PI).mult(-size/3));
    triangle(location.x, location.y, left.x, left.y, right.x, right.y);
  }
}
